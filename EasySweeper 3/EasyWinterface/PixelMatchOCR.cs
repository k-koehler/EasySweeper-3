...
namespace EasyWinterface {
    public class PixelMatchOCR {
        ...
        public List<string> readWinterface(Bitmap winterface) {
            var winterfaceList = WinterfaceOperations.chopWinterface(winterface);
            WinterfaceOperations.processList(ref winterfaceList);
        ...
        private static string _scanPixels(List<Tuple<Bitmap, string>> floorGlyphs, Bitmap bitmap, GLYPH_TYPE gt = GLYPH_TYPE.FLOOR) {
            ...
            for (var i = 0; i < bitmap.Width; ++i) {
                foreach (var glyph in floorGlyphs) {
                    if (WinterfaceOperations.FindRec(glyph.Item1, WinterfaceOperations.cropBitmap(bitmap, new Rectangle(i, 0, glyph.Item1.Width, glyph.Item1.Height))).Width == 0) {
